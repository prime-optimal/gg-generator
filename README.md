...
- **US address** generated locally with Faker — pin a state and get a **state-aligned ZIP**
  + full street address (randomuser can't filter by state and its ZIP doesn't match the state).
...
> Note: state + ZIP are aligned, but the **city** is plausible-but-random (not the real
> city for that ZIP). True city↔ZIP coherence needs a real ZIP dataset — easy to add.
...
(`REDACTED-VAULT-ID`) with username, password, email, full name, DOB,
phone, and address. Requires a signed-in `op`; use `--dry-run` to preview the command.
...
│   ├── address.py          # Faker US address, state-aligned ZIP
